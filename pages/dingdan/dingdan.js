...
Page({
  ...
  get_data_info: function() {
    ...
    // })
    wx.request({
      url: getApp().globalData.url + '/api/home/my_order_api',
      data: {
        uid: wx.getStorageSync('uid'),
      ...
      success: function(data) {
        ...
        if (data.data.code == 1) {
          if (that.data.current_page == 1) {
            that.setData({
              all_info_data: data.data.data.order_lst,
              sum_page: data.data.data.totalpage
            })
          } else {
            var new_page_cont = that.data.all_info_data;
            var current_guide_list = data.data.data.order_lst;
  ...
  dingxiang: function(e) {
    ...
    if (e.currentTarget.dataset.isdel==1){
      ...
      wx.navigateTo({
        url: '/pages/dingxiang/dingxiang?type=' + type + '&id=' + id
      })
    }
